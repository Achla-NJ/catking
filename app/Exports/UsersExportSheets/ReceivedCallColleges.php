...
class ReceivedCallColleges implements FromCollection, WithTitle, WithHeadings
{
    protected $options = [];
    protected $columns = [];
    protected $received_call_colleges = [];
    protected $colleges = [];
    protected $college_ids = [];
    protected $users = [];

    public function __construct($users, array $received_call_colleges)
    {
        $this->users = $users;
        $this->received_call_colleges = $received_call_colleges;
        $this->columns = [
            'user_id' => 'User ID',
            'user_name' => 'User Name',
        ...
        $this->colleges = College::query()
            ->whereIn('id', $this->received_call_colleges)
            ->where('id', '!=', '0')
        ...
        if(in_array(0,$received_call_colleges)){
    ...
    public function collection():? Collection
    {
        ...
        foreach ($users as $user) {
            ...
            $college_ids["rf_$user_id"] = $user->received_call_colleges->pluck('college_id')->toArray();
            foreach ($this->colleges as $college) {
                $result["rf_$user_id"]["college_$college->id"] = in_array($college->id, $college_ids["rf_$user_id"])? "Yes": "No";
            }
            $other_colleges["rf_$user_id"] = $user->received_call_colleges()
                ->join('colleges', 'colleges.id', 'student_received_call_colleges.college_id')
    ...
    {
        return 'Calls Received Colleges';
    }
}
